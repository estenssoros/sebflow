...
from sebflow.hooks.slack_hook import SlackHook
...
class SlackAPIOperator(BaseOperator):
    ...
    def __init__(self,
        ...
        if token is None and slack_conn_id is None:
            raise SebflowException('No valid Slack token nor slack_conn_id supplied.')
        if token is not None and slack_conn_id is not None:
            raise SebflowException('Cannot determine Slack credential when both token and slack_conn_id are supplied.')
...
class SlackAPIPostOperator(SlackAPIOperator):
    ...
    :type channel: string
    :param username: Username that sebflow will be posting to Slack as
