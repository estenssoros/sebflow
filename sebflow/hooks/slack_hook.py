    ...
    def __get_token(self, token, slack_conn_id):
        if token is not None:
            ...
            if not getattr(conn, 'password', None):
                raise SebflowException('Missing token(password) in Slack connection')
            return conn.password
        else:
            raise SebflowException('Cannot get token: No valid Slack token nor slack_conn_id supplied.')
